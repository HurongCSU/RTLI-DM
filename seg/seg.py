...
a = DataGenerator(train_mask, batch_size=64, shuffle=False)
...
model.compile(loss=FocalLoss, optimizer='adam', metrics=[dice_coef])
epochs = 70
batch_size = 16
...
validation_generator = DataGenerator(val_mask,
                                     ...
                                     img_size=img_size)
mm = keras.callbacks.ModelCheckpoint(os.path.join("./keras/model.h5"),
                                     monitor='val_loss',
                                     verbose=1,
                                     save_best_only=True,
                                     mode='min',
