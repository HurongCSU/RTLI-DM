...
def create_bags(input_data, input_labels, bag_count):
    # Set up bags.
    bags = []
    bag_labels = []
    ...
    for i in range(bag_count):
        ...
        instances_data = input_data[i]
        bag_label = input_labels[i]
...
for name, y_ in train:
    ...
    features_train.append(imgs)
    y_train.append(y_)

# Create training data.
train_data, train_labels = create_bags(
    features_train, y_train, BAG_COUNT
)
...
for name, y_ in val:
    tmp = sorted(glob(os.path.join(path, 'xy_all', str(name) + '_t2*.png')))
    imgs, _ = data_loader_tln(tmp, [y_] * len(tmp))
    features_val.append(imgs)
    y_val.append(y_)

# Create validation data.
val_data, val_labels = create_bags(
    features_val, y_val, VAL_BAG_COUNT
)
...
def create_model(instance_shape=(NET_SIZE, NET_SIZE, 3)):
    # Extract features from inputs.
    inputs, embeddings = [], []
    shared_dense_layer_1 = keras.applications.efficientnet.EfficientNetB4(weights='imagenet', include_top=False)
