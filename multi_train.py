...
from data_organize.utils import get_split, get_images, data_loader_tln, NET_SIZE, data_loader_tln_multi, \
    data_loader_tln_t1
...
class TrainDCNN:
    def __init__(self, optimizer=tf.keras.optimizers.SGD(lr=0.01, decay=0.00001)):
        ...
        self.save_model = ModelCheckpoint(
            "./model/T2_20231212_tln_roi_eff_multi.h5",
            monitor='val_loss',
            verbose=1,
            mode="min",
            save_best_only=True)
...
if __name__ == '__main__':
    ...
    train_set, y_train = data_loader_tln_t1(x_train, y_train)
    val_set, y_val = data_loader_tln_t1(x_val, y_val)
    test_fuyi, y_fuyi = data_loader_tln_t1(fuyi, y_fuyi)
    test_fuer, y_fuer = data_loader_tln_t1(fuer, y_fuer)
    test_chenyi, y_chenyi = data_loader_tln_t1(chenyi, y_chenyi)
